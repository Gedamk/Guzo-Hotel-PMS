...
def test_google_sheets_access():
    ...
    try:
        ...
        print("❌ credentials.json not found. Make sure it exists in the project root.")
        return
    ...
    def test_sheet(spreadsheet_id, sheet_name):
        try:
            ...
            print(f"✅ {sheet_name} Sheet Access Success!")
            print(f"Found {len(records)} records.")
            for r in records[:5]:  # print first 5 rows
                print(r)
        except gspread.SpreadsheetNotFound:
            print(f"❌ Spreadsheet not found: {sheet_name}. Check your spreadsheet ID and sharing settings.")
        except gspread.exceptions.APIError as e:
            print(f"❌ API error accessing {sheet_name}: {e}")
