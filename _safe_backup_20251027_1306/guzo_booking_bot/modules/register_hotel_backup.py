# -*- coding: utf-8 -*-
"""
register_hotel.py – Handles /register command for hotel managers
---------------------------------------------------------------
...
from guzo_booking_bot.modules import google_sheets   # must exist
...
async def register_hotel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not args:
        await update.message.reply_text("⚠️ Please provide a hotel name, e.g.\n/register Sky Light Hotel")
        return
    ...
    await update.message.reply_text(
        f"✅ Hotel registered successfully!\n�� {hotel_name}\n�� Manager: {user.full_name}\n�� {now}"
    )
