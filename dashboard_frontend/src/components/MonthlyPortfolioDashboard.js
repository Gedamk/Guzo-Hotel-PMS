...
function MonthlyPortfolioDashboard({ year, month }) {
  ...
  useEffect(() => {
    async function fetchAll() {
      try {
        ...
        const dreJson = dreRes.ok ? await dreRes.json() : null;
        ...
        const nnJson = nnRes.ok ? await nnRes.json() : null;
  ...
  if (loading) {
    return <div style={{ padding: "1rem" }}>Loading monthly reports for {monthLabel}…</div>;
  }
  ...
  return (
    <div style={{ padding: "1.5rem", fontFamily: "system-ui, sans-serif" }}>
      ...
      <p style={{ marginBottom: "1rem", color: "#555" }}>
        Period: {portfolio.report.period.start_date} → {portfolio.report.period.end_date}
      </p>
      ...
      >
        ...
        <KpiCard label="Room Nights" value={formatNumber(summary.room_nights_sold)} />
        <KpiCard label="Room Revenue (ETB)" value={formatNumber(summary.room_revenue_etb)} />
        <KpiCard label="ADR (ETB)" value={formatNumber(summary.adr)} />
        <KpiCard label="RevPAR (ETB)" value={formatNumber(summary.revpar)} />
        <KpiCard label="Occupancy" value={formatPercent(summary.occupancy_pct)} />
      </div>
      ...
      >
        {dre && (
          <HotelSummaryCard title="Dream Big Hotel (DRE001)" data={dre.summary} />
        )}
        {nn && (
          <HotelSummaryCard title="N&N Luxury Hotel (N&N002)" data={nn.summary} />
        )}
      </div>
    </div>
  );
...
function HotelSummaryCard({ title, data }) {
  const formatNumber = (value) =>
    Number(value).toLocaleString("en-US", { maximumFractionDigits: 2 });
...
export default MonthlyPortfolioDashboard;
