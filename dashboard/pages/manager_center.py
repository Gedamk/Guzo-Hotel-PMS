...
st.title(" Manager Center ÃÃ® Property Overview & Sync Control")
...
def check_google_sheets():
  try:
    ...
    ws = sheet.sheet1
    ws.row_values(1)
    return True, "ÃºÃ Connected successfully"
  except Exception as e:
    return False, f"Ã¹Ã¥ {e}"


def check_sendgrid():
  return (bool(SENDGRID_KEY), "ÃºÃ Key found" if SENDGRID_KEY else "Ã¹Ã¥ Missing SENDGRID_API_KEY")


def check_telegram():
  return (bool(TELEGRAM_TOKEN), "ÃºÃ Configured" if TELEGRAM_TOKEN else "Ã¹Ã¥ Missing TELEGRAM_BOT_TOKEN")


# ======================================================
# Â© STATUS DISPLAY
# ======================================================
st.subheader("Ã¥ Live Service Status")
...
st.caption(f" {g_msg} | ÃºÃ¢ {s_msg} | {t_msg}")
st.markdown("---")
...
# KPI OVERVIEW
# ======================================================
st.subheader("Ã  Operational KPIs")
c1, c2, c3, c4 = st.columns(4)
c1.metric("Â® Properties", 1)
c2.metric(" Messages", 25)
c3.metric("Â±â¢ Guests", 7)
c4.metric("Âµ Updated", datetime.now().strftime("%H:%M:%S"))
...
# Ã¶Ã´ MANUAL CONTROLS
# ======================================================
st.subheader("Ã¶Ã´ Manual Controls")
col1, col2 = st.columns(2)

with col1:
  if st.button("Ã Refresh Google Sheet Connection"):
    ok, msg = check_google_sheets()
    st.success(msg if ok else msg)

with col2:
  if st.button("Â§ Send Test Email (Simulated)"):
    st.info("ÃºÃ Test email sent successfully.")

st.markdown("---")
st.caption(f"Last checked: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | Â© 2025 Guzo Guest Assist")
